...
/** Formatos que funcionam por rampa de densidade: um caractere por célula. */
export function convertRamp(buf: PixelBuffer, options: ConvertOptions): string {
  ...
  for (let r = 0; r < grid.rows; r++) {
    ...
    for (let c = 0; c < grid.columns; c++) {
      ...
      let v = sampleLuminance(buf, x0, x0 + grid.cellWidth, y0, y0 + grid.cellHeight);
  ...
  // pixel claro (v alto) -> caractere esparso (índice baixo); pixel escuro -> denso
